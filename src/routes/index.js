...
import { Brush, CircuitBoard, Factory, FireExtinguisher, FlaskConical, Leaf, Pill, Snowflake, } from "lucide-react";
import xfi9 from "@/assets/xfi9-analyzer.jpg";
import { BookingButton } from "@/components/site/booking";
import { Container } from "@/components/site/container";
import { LogoMarquee } from "@/components/site/logo-marquee";
const DESCRIPTION = "CLM Industry réalise des campagnes de mesure de gaz industriels sur site. Identification ad nihilo de 500+ espèces gazeuses, conformité VLEP/CMR, optimisation des systèmes d'abattage. Analyseur XFLR-9 technologie OPO Laser.";
...
function Hero() {
    return (<section className="border-b border-border/40">
      <Container className="grid items-center gap-12 py-20 md:py-28 lg:grid-cols-2">
        ...
        <div className="relative">
          <div className="absolute inset-0 -z-10 blur-3xl" style={{
            background: "radial-gradient(circle at 50% 50%, rgba(33,150,182,0.35), transparent 70%)",
        }}/>
          <img src={xfi9} alt="Analyseur XFLR-9 — technologie OPO Laser pour la mesure de gaz industriels" width={1024} height={1024} className="relative w-full rounded-lg border border-border/60 shadow-2xl"/>
        </div>
      </Container>
    </section>);
}
const BenzeneSvg = () => (<svg width="100%" viewBox="0 0 120 120" role="img" aria-label="Molécule de benzène C6H6">
...
const FormaldehydeSvg = () => (<svg width="100%" viewBox="0 0 120 120" role="img" aria-label="Molécule de formaldéhyde CH2O">
    <g fill="none">
...
const TrichloroSvg = () => (<svg width="100%" viewBox="0 0 120 120" role="img" aria-label="Molécule de trichloroéthylène C2HCl3">
    <g fill="none">
    ...
    return (<section className="border-b border-border/40">
      <Container className="py-20 md:py-28">
        ...
        <div className="mt-14 grid gap-6 md:grid-cols-3">
          {molecules.map((m) => (<article key={m.name} className="card-hover flex flex-col items-center rounded-xl border border-border bg-card p-8 text-center">
...
function Solution() {
    ...
    return (<section className="border-b border-border/40 bg-[color:var(--footer)]">
      <Container className="py-20 md:py-28">
        <h2 className="max-w-3xl text-3xl font-bold leading-tight text-accent md:text-4xl">
        ...
        <ul className="mt-8 max-w-3xl space-y-5">
          {bullets.map(([label, text]) => (<li key={label} className="flex gap-3 text-base leading-relaxed text-muted-foreground">
        ...
        <div className="mt-12 grid gap-6 md:grid-cols-3">
          {stats.map(([n, l]) => (<div key={l} className="card-hover rounded-lg border border-border bg-card p-8">
        ...
        <Link to="/technologie" className="mt-10 inline-flex items-center gap-2 text-sm font-semibold text-accent transition-colors hover:text-foreground">
...
function Solutions() {
    ...
    return (<section id="solutions" className="scroll-mt-20 border-b border-border/40">
      <Container className="py-20 md:py-28">
        <h2 className="text-3xl font-bold leading-tight text-foreground md:text-4xl">
          Deux solutions adaptées à votre situation
        </h2>
        <div className="mt-12 grid gap-6 lg:grid-cols-2">
          {solutions.map((o) => (<article key={o.title} className="card-hover relative overflow-hidden rounded-lg border border-border bg-card p-8">
              ...
              <Link to={o.to} className="mt-8 inline-flex items-center gap-2 text-sm font-semibold text-accent transition-colors hover:text-foreground">
...
function Applications() {
    return (<section className="border-b border-border/40 bg-[color:var(--footer)]">
      <Container className="py-20 md:py-28">
        ...
        <Link to="/references" className="mt-6 inline-flex items-center gap-2 text-sm font-semibold text-accent transition-colors hover:text-foreground">
          Voir nos cas clients <span aria-hidden>→</span>
        </Link>
        <div className="mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {applications.map(({ Icon, title, desc }) => (<article key={title} className="group relative overflow-hidden rounded-xl border border-border bg-card p-6 text-center transition-colors hover:border-accent">
...
function FinalCta() {
    return (<section className="border-t-2 border-accent">
